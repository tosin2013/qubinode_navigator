...
class EquinixPlugin(QubiNodePlugin):
    ...
    def _initialize_plugin(self) -> None:
        ...
        self.equinix_tools = self.config.get('equinix_tools', [
            'metal-cli',  # Equinix Metal CLI
        ])
        
        # Bare metal specific packages
        self.metal_packages = self.config.get('metal_packages', [
            'curl', 'wget', 'jq', 'dmidecode', 'lshw', 'pciutils'
        ])
    ...
    def _install_equinix_cli(self) -> None:
        ...
        try:
            ...
            result = subprocess.run([
                'curl', '-s', 'https://api.github.com/repos/equinix/metal-cli/releases/latest'
            ], capture_output=True, text=True, check=True)
            ...
            subprocess.run([
                'curl', '-L', '-o', '/tmp/metal', download_url
            ], check=True)
            
            subprocess.run([
                'sudo', 'mv', '/tmp/metal', '/usr/local/bin/'
            ], check=True)
            
            subprocess.run([
                'sudo', 'chmod', '+x', '/usr/local/bin/metal'
            ], check=True)
...
"""
        ...
        subprocess.run([
            'sudo', 'mv', '/tmp/99-equinix-network.conf', '/etc/sysctl.d/'
        ], check=True)
...
"""
        ...
        subprocess.run([
            'sudo', 'mv', '/tmp/99-equinix-io.rules', '/etc/udev/rules.d/'
        ], check=True)
