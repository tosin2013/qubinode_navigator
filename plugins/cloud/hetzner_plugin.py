...
class HetznerPlugin(QubiNodePlugin):
    ...
    def _initialize_plugin(self) -> None:
        ...
        self.hetzner_tools = self.config.get('hetzner_tools', [
            'hcloud',  # Hetzner Cloud CLI
        ])
        
        # Cloud-specific packages
        self.cloud_packages = self.config.get('cloud_packages', [
            'curl', 'wget', 'jq', 'cloud-init', 'cloud-utils'
        ])
    ...
    def _install_hetzner_cli(self) -> None:
        ...
        try:
            ...
            subprocess.run([
                'curl', '-L', '-o', '/tmp/hcloud.tar.gz', download_url
            ], check=True)
...
"""
        ...
        subprocess.run([
            'sudo', 'mv', '/tmp/99-hetzner-network.conf', '/etc/sysctl.d/'
        ], check=True)
...
"""
        ...
        subprocess.run([
            'sudo', 'mv', '/tmp/99-hetzner-io.rules', '/etc/udev/rules.d/'
        ], check=True)
