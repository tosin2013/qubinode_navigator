...
class RHEL8Plugin(QubiNodePlugin):
    ...
    def _initialize_plugin(self) -> None:
        ...
        self.packages = self.config.get('packages', [
            'openssl-devel', 'bzip2-devel', 'libffi-devel', 'wget', 'vim', 'podman',
            'ncurses-devel', 'sqlite-devel', 'firewalld', 'make', 'gcc',
            'git', 'unzip', 'sshpass', 'lvm2', 'leapp-upgrade', 'cockpit-leapp',
            'java-11-openjdk', 'subscription-manager'
        ])
    ...
    def apply_changes(self, 
        ...
        try:
            ...
            if (desired_state.get('subscription_registered') and 
                not current_state.get('subscription_registered')):
            ...
            if (desired_state.get('subscription_attached') and 
                current_state.get('subscription_registered') and
                not current_state.get('subscription_attached')):
            ...
            if (desired_state.get('lab_user_exists') and 
                not current_state.get('lab_user_exists')):
    ...
    def _configure_python_upgrade(self) -> None:
        ...
        try:
            ...
            if version == "3.6.8":
                ...
                subprocess.run(['sudo', 'dnf', 'install', '-y', 'python39', 'python39-devel', 'python39-pip'], check=True)
                ...
                # Set alternatives
                subprocess.run(['sudo', 'alternatives', '--set', 'python', '/usr/bin/python3.9'], check=True)
                subprocess.run(['sudo', 'alternatives', '--set', 'python3', '/usr/bin/python3.9'], check=True)
    ...
    def _configure_podman_rootless(self) -> None:
        ...
        try:
            ...
            subprocess.run(['sudo', 'mv', '/tmp/xdg_runtime_dir.sh', '/etc/profile.d/xdg_runtime_dir.sh'], check=True)
            subprocess.run(['sudo', 'chmod', 'a+rx', '/etc/profile.d/xdg_runtime_dir.sh'], check=True)
            subprocess.run(['sudo', 'cp', '/etc/profile.d/xdg_runtime_dir.sh', '/etc/profile.d/xdg_runtime_dir.zsh'], check=True)
            ...
            subprocess.run(['sudo', 'mv', '/tmp/ping_group_range.conf', '/etc/sysctl.d/ping_group_range.conf'], check=True)
    ...
    def _setup_qubinode_navigator(self) -> None:
        ...
        try:
            ...
            if os.path.exists(nav_dir):
                ...
                # Create symlink
                subprocess.run(['sudo', 'usermod', '-aG', 'users', os.getenv('USER', 'root')], check=True)
                ...
                if not os.path.exists('/opt/qubinode_navigator'):
                    subprocess.run(['sudo', 'ln', '-s', nav_dir, '/opt/qubinode_navigator'], check=True)
...
"""
    ...
    def _install_development_tools(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(['dnf', 'group', 'info', 'Development Tools'], 
                                  capture_output=True, text=True)
            
            if 'Installed Groups:' not in result.stdout:
                self.logger.info("Installing Development Tools group")
                subprocess.run(['sudo', 'dnf', 'groupinstall', 'Development Tools', '-y'], check=True)
...
gpgkey=https://packages.microsoft.com/keys/microsoft.asc"""
                ...
                subprocess.run(['sudo', 'mv', '/tmp/vscode.repo', '/etc/yum.repos.d/vscode.repo'], check=True)
