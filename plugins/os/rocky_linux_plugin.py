...
class RockyLinuxPlugin(QubiNodePlugin):
    ...
    def _initialize_plugin(self) -> None:
        ...
        self.packages = self.config.get('packages', [
            ...
            'git', 'unzip', 'sshpass', 'lvm', 'lvm2', 'zlib-devel',
            'python3.11', 'python3.11-pip', 'python3.11-devel', 'java-11-openjdk'
        ])
    ...
    def apply_changes(self, 
        ...
        try:
            ...
            if (not current_state.get('ansible_navigator_configured') and 
                desired_state.get('ansible_navigator_configured')):
    ...
    def _configure_firewalld(self) -> None:
        ...
        try:
            ...
            subprocess.run(['sudo', 'firewall-cmd', '--permanent', '--add-service=http'], check=True)
            subprocess.run(['sudo', 'firewall-cmd', '--permanent', '--add-service=https'], check=True)
