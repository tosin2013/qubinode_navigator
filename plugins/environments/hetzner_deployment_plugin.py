...
class HetznerDeploymentPlugin(QubiNodePlugin):
    ...
    def _initialize_plugin(self) -> None:
        ...
        self.required_packages = self.config.get('required_packages', [
            'curl', 'wget', 'git', 'vim', 'openssh-clients'
        ])
    ...
    def _is_hetzner_environment(self) -> bool:
        ...
        hetzner_indicators = [
            ...
            lambda: any(pattern in subprocess.run(['hostname'], capture_output=True, text=True).stdout.lower() 
    ...
    def _is_lab_user_ssh_configured(self) -> bool:
        ...
        return (os.path.exists(ssh_key_path) and 
                os.path.exists(authorized_keys_path))
    ...
    def _download_configure_script(self) -> None:
        ...
        try:
            subprocess.run([
                'curl', '-OL', self.configure_script_url
            ], check=True)
    ...
    def _configure_lab_user_ssh(self) -> None:
        ...
        try:
            ...
            subprocess.run([
                'sudo', '-u', 'lab-user', 'ssh-keygen', 
                '-f', f"{ssh_dir}/id_rsa", '-t', 'rsa', '-N', ''
            ], check=True)
    ...
    def _get_next_steps(self) -> List[str]:
        ...
        next_steps.extend([
            "Run: source notouch.env",
            "Continue with Qubinode Navigator deployment"
        ])
