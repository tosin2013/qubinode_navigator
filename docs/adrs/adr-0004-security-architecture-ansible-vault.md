...
## layout: default title: ADR-0004 Security Architecture parent: Security & Operations grand_parent: Architectural Decision Records nav_order: 4
...
1. **Environment variables only** - Insufficient for complex multi-environment scenarios
1. **External secret management systems (HashiCorp Vault, AWS Secrets Manager)** - Too complex for current requirements
1. **Ansible Vault alone** - Enhanced with AnsibleSafe for better usability and security
1. **Encrypted configuration files with custom tooling** - Ansible Vault chosen for ecosystem integration
...
1. **Storage**: Credentials encrypted and stored in environment-specific vault files
1. **Access**: Ansible automatically decrypts during playbook execution
1. **Rotation**: Manual process using AnsibleSafe to update vault files
1. **Audit**: Git history provides audit trail of vault file changes
