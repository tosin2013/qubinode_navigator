...
class FailureAnalyzer:
    ...
    def analyze(self, diagnostic_context: Any) -> FailureAnalysis:
        ...
        self.logger.info(f"Analysis complete: {analysis.root_cause.value} " f"(confidence: {analysis.confidence:.2%})")
    ...
    def _load_error_patterns(self) -> List[KnownErrorPattern]:
        ...
        patterns = [
            ...
            KnownErrorPattern(
                ...
                recovery_hint="Start libvirtd: systemctl start libvirtd or podman restart <container>",
            ),
            # Network errors
            KnownErrorPattern(
            ...
            # Airflow-specific errors (now container-aware)
            KnownErrorPattern(
                ...
                recovery_hint="For containerized Airflow: podman restart <airflow-container> or see container logs with podman logs <container>",
            ),
    ...
    def _analyze_service_dependencies(self, analysis: FailureAnalysis, diagnostic_context: Any) -> None:
        """Analyze service health to correlate with failure (supports containers and systemd)"""
        ...
        if unhealthy_services:
            ...
            for service in unhealthy_services:
                if any(crit in service for crit in critical_services) and analysis.confidence < 0.9:
                    ...
                    # Generate container-aware recovery action
                    recovery_action = self._generate_service_recovery_action(service, health_info)
                    analysis.recommended_actions.insert(0, recovery_action)

    def _generate_service_recovery_action(self, service_name: str, health_info: Dict[str, Any]) -> str:
        """
        Generate a recovery action that accounts for container vs systemd services

        Args:
            service_name: Name of the service
            health_info: Service health dict (may include is_container flag)

        Returns:
            String with appropriate recovery command
        """
        is_container = health_info.get("is_container", False)
        container_name = health_info.get("container_name")

        if is_container and container_name:
            # Service is containerized
            if service_name == "airflow":
                # Airflow has multiple containers
                return f"Restart Airflow containers: podman restart {container_name} airflow-scheduler airflow-webserver"
            elif service_name == "postgres":
                return f"Restart database container: podman restart {container_name}"
            else:
                return f"Restart container: podman restart {container_name}"
        else:
            # Service is managed by systemd
            if service_name == "airflow":
                return "Start Airflow service: systemctl start airflow (or restart containers if using Podman)"
            else:
                return f"Start service: systemctl start {service_name}"
...
Be concise and actionable.""",
    ...
    def _apply_llm_reasoning(self, analysis: FailureAnalysis, diagnostic_context: Any) -> None:
        ...
        try:
            ...
            self.logger.debug("Invoking LLM reasoning agent")
    ...
    def _generate_recommendations(self, analysis: FailureAnalysis) -> None:
        ...
        if not analysis.recommended_actions:
            analysis.recommended_actions = ["Check service logs: journalctl -xe", "Verify system resources: free -h; df -h", "Check network: ping -c 3 8.8.8.8"]
