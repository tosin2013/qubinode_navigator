...
class LogAnalyzer:
    ...
    def analyze_deployment_session(self, log_entries: List[LogEntry]) -> DeploymentSession:
        ...
        end_entry = next((e for e in reversed(log_entries) if e.event_type == 'deployment_complete'), None)
    ...
    def generate_resolution_recommendations(self, session: DeploymentSession, error_patterns: List[str]) -> List[ResolutionRecommendation]:
        ...
        recommendations.sort(key=lambda r: (r.confidence, self._severity_weight(r.error_pattern_id)), reverse=True)
    ...
    def generate_analysis_report(self, session: DeploymentSession, error_patterns: List[str], 
                               recommendations: List[ResolutionRecommendation], 
                               ai_analysis: str = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            'error_analysis': {
                ...
                'pattern_details': [
                    ...
                    for pid in error_patterns if pid in self.error_patterns
                ]
            },
