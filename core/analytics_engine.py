...
from core.monitoring_manager import Alert, Metric, MonitoringManager
from core.rollback_manager import RollbackManager, RollbackStatus
from core.rollout_pipeline import PipelineStage, RolloutPipeline, RolloutStrategy
...
class AnalyticsEngine:
    ...
    def _analyze_failures(self, pipelines: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        failure_analysis = {
            ...
            "failure_rate": (
                round((len(failed_pipelines) / len(pipelines) * 100), 2)
                if pipelines
                else 0
            ),
