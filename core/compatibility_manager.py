...
class CompatibilityManager:
        ...
        try:
                ...
                if rule.component_name == component and os_version in rule.os_versions:
                    if self._evaluate_rule(rule, version):
                        if rule.rule_type == 'exclusion':
                            return CompatibilityLevel.INCOMPATIBLE, f"Excluded by rule: {rule.description}"
                        elif rule.rule_type == 'requirement':
                            return CompatibilityLevel.COMPATIBLE, f"Meets requirement: {rule.description}"
            ...
            if recent_results:
                ...
                if passed / total >= 0.8:  # 80% pass rate
                    return CompatibilityLevel.COMPATIBLE, f"Recent tests: {passed}/{total} passed"
                elif passed / total < 0.5:  # Less than 50% pass rate
                    return CompatibilityLevel.INCOMPATIBLE, f"Recent tests: {passed}/{total} passed"
            
            return CompatibilityLevel.NEEDS_TESTING, "No compatibility information available"
    ...
    def _get_recent_test_results(self, component: str, version: str, os_version: str, days: int = 30) -> List[TestResult]:
        ...
        for result in self.test_results[component]:
            if (result.component_version == version and 
                result.os_version == os_version and 
    ...
    async def _test_podman_compatibility(self, version: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(['podman', 'run', '--rm', 'hello-world'], capture_output=True, text=True, timeout=60)
            if result.returncode != 0:
  ...
  tasks:
    ...
    async def _test_git_compatibility(self, version: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Initialize repo
            result = subprocess.run(['git', 'init'], cwd=test_dir, capture_output=True, text=True, timeout=30)
            if result.returncode != 0:
            ...
            result = subprocess.run(['git', 'add', 'test.txt'], cwd=test_dir, capture_output=True, text=True, timeout=30)
            if result.returncode != 0:
