...
from core.update_validator import UpdateValidator, ValidationResult
...
class AIUpdateManager:
    ...
    async def analyze_update_with_ai(self, update_info: UpdateInfo) -> AIAnalysis:
        ...
        try:
            ...
            compatibility_level, compatibility_reason = await self.compatibility_manager.validate_compatibility(
    ...
    def _create_analysis_prompt(self, update_info: UpdateInfo, compatibility_level: CompatibilityLevel, compatibility_reason: str) -> str:
...
"""
    ...
    def _create_fallback_analysis(self, analysis_id: str, update_info: UpdateInfo) -> AIAnalysis:
        ...
        return AIAnalysis(
            ...
            testing_recommendations=["Perform standard validation tests", "Monitor system after update"],
        ...
        for update in batch.updates:
            try:
                ...
                fallback = self._create_fallback_analysis(
                    f"fallback_{update.component_name}_{int(time.time())}", 
                    update
                )
    ...
    def _create_execution_phases(self, batch: UpdateBatch, analyses: Dict[str, AIAnalysis], strategy: str) -> List[Dict[str, Any]]:
        ...
        if strategy == "immediate":
            ...
            low_risk_updates = [
                update.component_name for update in batch.updates 
                if analyses[update.component_name].risk_level in [RiskLevel.VERY_LOW, RiskLevel.LOW]
            ]
            ...
            higher_risk_updates = [
                update.component_name for update in batch.updates 
                if analyses[update.component_name].risk_level in [RiskLevel.MEDIUM, RiskLevel.HIGH]
            ]
            ...
            phases.append({
                ...
                "tasks": ["System backup", "Service status check", "Notification sent"]
            })
            ...
            phases.append({
                ...
                "tasks": ["System validation", "Service verification", "Performance check"]
            })
        ...
        for update in updates:
            try:
                ...
                recommendations[update.component_name] = {
                    ...
                    'should_auto_approve': (
                        analysis.confidence_score >= self.auto_approval_threshold and
                        analysis.risk_level in [RiskLevel.VERY_LOW, RiskLevel.LOW] and
                        analysis.recommendation in [UpdateRecommendation.APPLY_IMMEDIATELY, UpdateRecommendation.APPLY_WITH_TESTING]
                    )
                }
    ...
    async def generate_update_report(self, batch_id: str) -> Dict[str, Any]:
        ...
        batch_analyses = {
            ...
            if analysis.analysis_id.startswith(f"ai_analysis_") and batch_id in analysis.analysis_id
        }
        ...
        report = {
            ...
            'risk_assessment': {
                ...
                'high_risk_components': [
                    name for name, analysis in batch_analyses.items() 
                    if analysis.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]
                ],
                'security_updates': [
                    name for name, analysis in batch_analyses.items() 
