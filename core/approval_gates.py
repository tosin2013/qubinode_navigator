...
from core.rollout_pipeline import ApprovalGate, ApprovalStatus, PipelineStage
...
class ApprovalGateManager:
    ...
    def _load_auto_approval_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'allowed_components': ['git', 'python', 'nodejs'],  # Components safe for auto-approval
    ...
    async def check_gate_approval_status(self, gate: ApprovalGate) -> Tuple[ApprovalStatus, str]:
        ...
        if gate.status in [ApprovalStatus.APPROVED, ApprovalStatus.REJECTED, ApprovalStatus.AUTO_APPROVED]:
        ...
        if approvals >= required_count:
            ...
            gate.approved_at = datetime.now()
            return ApprovalStatus.APPROVED, f"Gate approved by {approvals}/{required_count} approvers"
        
        # Still pending
        return ApprovalStatus.PENDING, f"Waiting for {required_count - approvals} more approval(s)"
