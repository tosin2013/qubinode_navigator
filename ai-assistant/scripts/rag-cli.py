...
def cmd_query(args):
    ...
    if rag is None:
        print(json.dumps({"status": "error", "message": "RAG service not available. Install qdrant-client or chromadb."}))
        return 1
...
Examples:
  ...
  %(prog)s health
    ...
    commands = {"ingest": cmd_ingest, "query": cmd_query, "stats": cmd_stats, "list": cmd_list, "health": cmd_health}
