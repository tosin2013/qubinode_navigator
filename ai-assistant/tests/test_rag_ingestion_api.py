...
# TEST_MODE is set in conftest.py before any imports
...
class TestValidationResult:
    ...
    def test_validation_result_valid(self):
        ...
        result = ValidationResult(is_valid=True, quality_score=0.9, issues=[], suggestions=[])
    ...
    def test_validation_result_invalid(self):
        ...
        result = ValidationResult(is_valid=False, quality_score=0.4, issues=["Content too short"], suggestions=["Add more details"])
...
class TestProcessedChunk:
    ...
    def test_chunk_creation(self):
        ...
        chunk = ProcessedChunk(id="chunk_001", content="Test content for chunk", title="Test Title", metadata={"category": "test"}, quality_score=0.8)
...
class TestRAGIngestionServiceValidation:
    ...
    async def test_validate_content_low_quality(self, tmp_path):
        ...
        chunks = [ProcessedChunk(id="test_1", content="Very short no technical terms no code examples.", title="Title", metadata={}, quality_score=0.0)]
...
class TestRAGIngestionServiceQueueing:
    ...
    async def test_queue_for_review(self, tmp_path):
        ...
        doc_id = "test_123"
        chunks = [ProcessedChunk(id="chunk_1", content="Test content", title="Test", metadata={}, quality_score=0.7)]
        metadata = DocumentMetadata(category="test")
...
class TestRAGIngestionServiceAddToDatabase:
    ...
    async def test_add_to_rag_database(self, tmp_path):
        ...
        chunks = [ProcessedChunk(id="chunk_1", content="Test content for RAG database", title="Test Title", metadata={"category": "test"}, quality_score=0.8)]
...
Enable and start libvirtd service for VM management."""
        ...
        mock_file = MockFile()
        metadata = DocumentMetadata(category="deployment", tags=["kvm", "rhel"], difficulty_level="intermediate")
        ...
        with patch.object(service, "_add_to_rag_database", new_callable=AsyncMock):
            response = await service.ingest_document(file=mock_file, metadata=metadata, validate=True, auto_approve=True)
    ...
    async def test_ingest_document_low_quality_rejected(self, tmp_path):
        ...
        with pytest.raises(Exception):  # HTTPException or wrapped exception
            await service.ingest_document(file=mock_file, metadata=metadata, validate=True)
...
It provides some value but could be improved with more details."""
            ...
            response = await service.ingest_document(file=mock_file, metadata=metadata, validate=True, auto_approve=False)
    ...
    async def test_ingest_document_no_validation(self, tmp_path):
        ...
        with patch.object(service, "_add_to_rag_database", new_callable=AsyncMock):
            ...
            response = await service.ingest_document(file=mock_file, metadata=metadata, validate=False, auto_approve=True)
