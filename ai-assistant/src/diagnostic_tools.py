...
class DiagnosticTool(ABC):
    ...
    def _create_result(self, success: bool, data: Dict[str, Any], 
                      error: Optional[str] = None, execution_time: float = 0.0) -> ToolResult:
...
class ServiceStatusTool(DiagnosticTool):
    ...
    async def execute(self, services: Optional[List[str]] = None, **kwargs) -> ToolResult:
        ...
        default_services = [
            "libvirtd", "qemu-kvm", "NetworkManager", "firewalld", 
            "sshd", "chronyd", "systemd-resolved", "dnsmasq"
        ]
            ...
            for service in services_to_check:
                try:
                    ...
                    info_result = subprocess.run(
                        ["systemctl", "show", service, "--property=LoadState,ActiveState,SubState"],
                        capture_output=True,
                        text=True,
                        timeout=5
                    )
                    ...
                    service_status[service] = {"status": "timeout", "active": False, "error": "Command timeout"}
                except Exception as e:
                    service_status[service] = {"status": "error", "active": False, "error": str(e)}
        ...
        try:
            ...
            for target in test_targets:
                try:
                    ...
                    connectivity_results[target] = {"reachable": False, "error": "timeout"}
                except Exception as e:
