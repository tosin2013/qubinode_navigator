...
class TestAIAssistantIntegration:
    ...
    def setup_class(cls):
        ...
        try:
            ...
            result = subprocess.run(['docker', 'images', '-q', cls.image_name], 
            ...
            start_cmd = [
                'docker', 'run', '-d', '--name', cls.container_name,
                '-p', f'{cls.port}:8080',
                # Environment variables for faster startup
                '-e', 'AI_MODEL_TYPE=granite-4.0-micro',  # Use the model we know works
                '-e', 'AI_TEST_MODE=false',  # Use real mode but with optimizations
                '-e', 'AI_LOG_LEVEL=info',
                '-e', 'AI_THREADS=2',  # Reduce threads for faster startup
                '-e', 'AI_CONTEXT_LENGTH=2048',  # Smaller context for faster startup
                '-e', 'AI_MAX_TOKENS=256',  # Smaller response for faster testing
                # Health check configuration
                '--health-cmd', f'curl -f --max-time 30 http://localhost:8080/health || exit 1',
                '--health-interval', '15s',
                '--health-timeout', '30s',
                '--health-retries', '10',
                '--health-start-period', '120s',  # Give 2 minutes for initial startup
                cls.image_name
            ]
            ...
            for i in range(max_attempts):
                ...
                try:
                    ...
                    if i > 0 and i % 12 == 0:
                        ...
                        log_result = subprocess.run(['docker', 'logs', '--tail', '5', cls.container_name], 
                                                  capture_output=True, text=True, check=False)
            ...
            log_result = subprocess.run(['docker', 'logs', '--tail', '100', cls.container_name], 
                                      capture_output=True, text=True, check=False)
            ...
            status_result = subprocess.run(['docker', 'ps', '-a', '--filter', f'name={cls.container_name}'], 
                                         capture_output=True, text=True, check=False)
        ...
        for attempt in range(max_retries):
            try:
                response = requests.post(f'{self.base_url}/chat',
                    json={'message': 'What is KVM virtualization?'}, timeout=60)
