...
from core.rollout_pipeline import RolloutPipelineManager, RolloutStrategy, PipelineStage
...
async def approve_command(args):
    ...
    try:
        ...
        if success:
            ...
            print(f"❌ Failed to process approval response")
...
Examples:
  ...
  python3 rollout_pipeline.py approve --request-id req_123 --response approved
    ...
    create_parser.add_argument(
        '--created-by',
        default='cli-user',
        help='Pipeline creator (default: cli-user)'
    )
    create_parser.add_argument(
        '--plan-file',
        help='JSON file containing update plan'
    )
    create_parser.add_argument(
        '--max-updates',
        type=int,
    ...
    list_parser.add_argument(
        '--status',
        choices=['pending', 'approved', 'deploying', 'validating', 'completed', 'failed', 'rolled_back', 'cancelled'],
        help='Filter by pipeline status'
    )
    list_parser.add_argument(
        '--include-completed',
        action='store_true',
        help='Include completed pipelines'
    )
    list_parser.add_argument(
        '--limit',
        type=int,
        default=10,
        help='Maximum pipelines to show (default: 10)'
    )
