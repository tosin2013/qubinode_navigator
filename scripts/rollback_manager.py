...
from core.rollback_manager import RollbackManager, RollbackTrigger, RollbackStatus
...
Examples:
  ...
  python rollback_manager.py list --format json
    ...
    stats_parser.add_argument('--format', choices=['summary', 'json'], default='summary',
                             help='Output format')
    
    # Trigger command
    trigger_parser = subparsers.add_parser('trigger', help='Trigger manual rollback')
    trigger_parser.add_argument('--pipeline-id', required=True,
                               help='Pipeline ID to rollback')
    trigger_parser.add_argument('--reason', required=True,
                               help='Reason for rollback')
    trigger_parser.add_argument('--phase-id', 
                               help='Specific phase ID to rollback (optional)')
    trigger_parser.add_argument('--requested-by', default='cli-user',
                               help='User requesting rollback')
    
    # Details command
    details_parser = subparsers.add_parser('details', help='Get rollback plan details')
    details_parser.add_argument('--plan-id', required=True,
                               help='Rollback plan ID')
    details_parser.add_argument('--format', choices=['summary', 'json'], default='summary',
                               help='Output format')
    
    # List command
    list_parser = subparsers.add_parser('list', help='List all rollbacks')
    list_parser.add_argument('--status', choices=['active', 'completed', 'failed', 'all'], 
    ...
    test_parser.add_argument('--component', choices=['manager', 'triggers', 'execution'], 
                            default='manager', help='Component to test')
