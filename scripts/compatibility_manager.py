...
from core.compatibility_manager import CompatibilityManager, CompatibilityLevel, TestStatus
    ...
    for component, details in report['components'].items():
        ...
        if details['supported_versions']:
            ...
            for os_ver, versions in details['supported_versions'].items():
                print(f"        OS {os_ver}: {', '.join(versions[:3])}" + 
...
Examples:
  ...
  python3 compatibility_manager.py update
    ...
    # Update command
    update_parser = subparsers.add_parser('update', help='Update compatibility matrices')
