...
Examples:
  ...
  python performance_optimizer.py test --component monitoring
    ...
    status_parser.add_argument('--format', choices=['summary', 'json'], default='summary',
                              help='Output format')
    ...
    monitor_parser.add_argument('--optimization-level', choices=['conservative', 'balanced', 'aggressive'],
    ...
    rec_parser.add_argument('--impact', choices=['low', 'medium', 'high'],
                           help='Filter by impact level')
    rec_parser.add_argument('--format', choices=['table', 'json'], default='table',
                           help='Output format')
    
    # Optimize command
    opt_parser = subparsers.add_parser('optimize', help='Apply optimization')
    opt_parser.add_argument('--recommendation-id', required=True,
    ...
    test_parser.add_argument('--component', choices=['monitoring', 'optimization', 'caching'],
                            default='monitoring', help='Component to test')
    
    # Metrics command
    metrics_parser = subparsers.add_parser('metrics', help='Show performance metrics')
    metrics_parser.add_argument('--hours', type=int, default=1,
                               help='Hours of metrics to show')
    metrics_parser.add_argument('--format', choices=['summary', 'json'], default='summary',
                               help='Output format')
        ...
        if args.format == 'json':
            ...
            if summary['status'] == 'active':
                ...
                for resource, limits in summary['resource_limits'].items():
                    status_symbol = {'normal': '✓', 'warning': '⚠', 'critical': '✗'}.get(limits['status'], '•')
        ...
        while (asyncio.get_event_loop().time() - start_time) < args.duration:
            ...
            if summary['status'] == 'active':
                ...
                print(f"\r[{datetime.now().strftime('%H:%M:%S')}] "
                      ...
                      f"PROC: {metrics['process_count']}", end='', flush=True)
...
async def handle_recommendations_command(optimizer: PerformanceOptimizer, args):
    ...
    try:
        ...
        if args.format == 'json':
            ...
            for rec in recommendations:
                print(f"{rec.recommendation_id:<20} {rec.resource_type.value:<10} {rec.impact_level:<8} "
        ...
        if args.format == 'json':
            ...
            if optimizer.performance_history:
                ...
                for metric in recent:
                    ...
                    print(f"{time_str:<20} {metric.cpu_usage:<8.1f} {metric.memory_usage:<10.1f} "
