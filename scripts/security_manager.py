...
from core.security_manager import SecurityManager, SecurityLevel, AccessLevel
...
Examples:
  ...
  python security_manager.py resolve --vulnerability-id vuln_123
    ...
    token_parser.add_argument('--action', choices=['generate', 'validate', 'revoke'], 
                             default='generate', help='Token action')
    token_parser.add_argument('--user', required=True, help='User ID')
    token_parser.add_argument('--level', choices=['read', 'write', 'admin', 'super_admin'],
    ...
    vuln_parser.add_argument('--severity', choices=['low', 'medium', 'high', 'critical'],
                            help='Filter by severity')
    vuln_parser.add_argument('--format', choices=['table', 'json'], default='table',
                            help='Output format')
    vuln_parser.add_argument('--resolved', action='store_true',
                            help='Include resolved vulnerabilities')
    
    # Resolve command
    resolve_parser = subparsers.add_parser('resolve', help='Resolve vulnerability')
    resolve_parser.add_argument('--vulnerability-id', required=True,
                               help='Vulnerability ID to resolve')
    ...
    test_parser.add_argument('--component', choices=['auth', 'encryption', 'scanning'],
                            default='auth', help='Component to test')
...
async def handle_scan_command(security_manager: SecurityManager, args):
    ...
    try:
        ...
        if args.format == 'json':
            ...
            for vuln in vulnerabilities:
                severity_symbol = {'critical': '🔴', 'high': '🟠', 'medium': '🟡', 'low': '🟢'}.get(vuln.severity.value, '•')
