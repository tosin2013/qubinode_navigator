...
1. **Find DAG**: `example_kcli_vm_provisioning`
1. **Click Play**: Trigger the DAG
1. **Watch Progress**: Go to Graph view
...
1. **Adjust the 5-minute wait** (change `sleep 300` in the DAG)
1. **Add more validation steps** during the wait period
1. **Build multi-VM workflows** using parallel tasks
