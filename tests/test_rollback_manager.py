...
from core.rollback_manager import (
    RollbackManager, RollbackPlan, RollbackAction, RollbackTrigger, RollbackStatus
)
from core.rollout_pipeline import RolloutPipeline, RolloutPhase, PipelineStage, RolloutStrategy
...
class TestRollbackManager:
    ...
    async def test_monitor_pipeline_health(self):
        ...
        with mock.patch.object(self.rollback_manager, '_check_deployment_failure', return_value=False):
            with mock.patch.object(self.rollback_manager, '_check_critical_system_failure', return_value=False):
                with mock.patch.object(self.rollback_manager, '_check_service_availability', return_value=False):
                    with mock.patch.object(self.rollback_manager, '_check_error_rates', return_value=False):
                        with mock.patch.object(self.rollback_manager, '_check_performance_degradation', return_value=False):
    ...
    async def test_validate_rollback_success(self):
        ...
        with mock.patch.object(self.rollback_manager, '_check_system_health', 
                              return_value={'status': 'passed'}):
            with mock.patch.object(self.rollback_manager, '_check_service_health', 
                                  return_value={'status': 'passed'}):
                with mock.patch.object(self.rollback_manager, '_check_performance_metrics', 
                                      return_value={'status': 'passed'}):
