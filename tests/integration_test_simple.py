    ...
    def setup_class(cls):
        ...
        try:
            ...
            start_cmd = [
                ...
                '-e', 'AI_LOG_LEVEL=info',
                '-e', 'AI_THREADS=2',
                '-e', 'AI_CONTEXT_LENGTH=1024',  # Smaller for faster startup
                '-e', 'AI_MAX_TOKENS=128',
                # Disable features that might cause health check failures
                '-e', 'AI_RAG_ENABLED=false',  # Disable RAG for faster startup
                '-e', 'AI_DIAGNOSTICS_ENABLED=true',
                cls.image_name
            ]
            ...
            for i in range(max_attempts):
                ...
                try:
                    ...
                    if i > 0 and i % 12 == 0:
                        ...
                        log_result = subprocess.run(['docker', 'logs', '--tail', '10', cls.container_name], 
                                                  capture_output=True, text=True, check=False)
            ...
            log_result = subprocess.run(['docker', 'logs', cls.container_name], 
                                      capture_output=True, text=True, check=False)
