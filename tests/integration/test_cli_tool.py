...
import qubinode_cli
from core import PluginManager, EventSystem, ConfigManager
...
""")
    ...
    def test_list_plugins_with_plugins(self, mock_plugin_manager_class):
        ...
        mock_plugin_manager = Mock()
        mock_plugin_manager.list_plugins.return_value = ["test_plugin", "another_plugin"]
    ...
    def test_plugin_discovery_integration(self, mock_plugin_manager_class):
        ...
        mock_plugin_manager = Mock()
        mock_plugin_manager.discover_plugins.return_value = ["rhel9_plugin", "hetzner_plugin"]
        mock_plugin_manager.list_plugins.return_value = ["rhel9_plugin", "hetzner_plugin"]
    ...
    def test_inventory_parameter(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(cli_path), 'execute', '--inventory', 'test_inventory'],
                capture_output=True,
                text=True,
                timeout=10,
                cwd=str(Path(__file__).parent.parent.parent)
